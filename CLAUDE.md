...
- **Staging is decided by a declared production host, nothing else.** The site declares its production host once, on production (Valolink → Staging, "Use current site URL"); it is stored as a SHA-256 hash plus a plaintext copy for display, so a clone's search-replace cannot rewrite it. `StagingDetector::decision()` = force flag → undeclared (not staging; the module nags on every admin screen) → `hash(home host) ≠ declaration` (staging) → production. The mu-loader mirrors the decision inline with no classes; `php tests/staging-decision.php` holds the two to the same answers — run it after touching either. The old heuristics survive only as `looks_like_staging()`, advisory, used to refuse declaring a host that looks like a clone. No legacy fallback by design. The installed loader is refreshed from `admin_init`, not only on activation.
