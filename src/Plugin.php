...
final class Plugin
{
    ...
    public static function register_modules(Registry $registry, Settings $settings): void
    {
        $registry->register(new ModuleManifest(
            ...
            label: static fn (): string => __('Agency Branding', 'valolink-plugin'),
            description: static fn (): string => __('Replace the WordPress login logo and add agency support contact info beneath the login form.', 'valolink-plugin'),
            class: BrandingModule::class,
        ));

        $registry->register(new ModuleManifest(
            id: StagingModule::MODULE_ID,
            label: static fn (): string => __('Staging', 'valolink-plugin'),
            description: static fn (): string => __('Detects staging environments (or forces staging mode) and lets you switch on per-feature controls: block indexing, intercept mail, disable Woo gateways, require login, redirect to a "coming soon" page, disable specific plugins, and block auto-updates. Configure under Valolink → Staging.', 'valolink-plugin'),
        ...
        $registry->register(new ModuleManifest(
            ...
            label: static fn (): string => __('Event Log', 'valolink-plugin'),
            description: static fn (): string => __('Records site events (logins, plugin/theme changes, user changes, publishes) into a local table. Exposed to EngineLink via the same REST namespace. Pruned daily; retention configurable.', 'valolink-plugin'),
        ...
        $registry->register(new ModuleManifest(
            ...
            label: static fn (): string => __('Email (Resend)', 'valolink-plugin'),
            description: static fn (): string => __('Routes wp_mail() through Resend\'s HTTPS API — avoids blocked SMTP ports. Force From email/name, default Reply-To, BCC catch-all, fallback on failure, admin alerts, and a one-click test send. Per-message logging goes through the Event Log module. Configure under Valolink → Email.', 'valolink-plugin'),
        ...
        $registry->register(new ModuleManifest(
            ...
            label: static fn (): string => __('Security', 'valolink-plugin'),
            description: static fn (): string => __('Per-feature hardening toggles (disable XML-RPC, etc.). Configure under Valolink → Security.', 'valolink-plugin'),
        ...
        $registry->register(new ModuleManifest(
            ...
            label: static fn (): string => __('Toolbox', 'valolink-plugin'),
            description: static fn (): string => __('Grab-bag of small toggles (allow SVG uploads, …) that don\'t justify a module of their own. Configure under Valolink → Toolbox.', 'valolink-plugin'),
